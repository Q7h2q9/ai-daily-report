...
# 运行时由 build_pages_site() 从 settings 初始化
_SITE_URL: str = ""
_REPO_URL: str = ""
...
def _build_json_feed(reports: list[dict[str, object]], site_url: str = "") -> dict[str, object]:
    items = []
    for report in reports:
...
def _build_rss(reports: list[dict[str, object]], site_url: str = "") -> str:
    items: list[str] = []
    for report in reports:
        summary = " ".join(report["executive_summary"]) or report["judgment"]
        items.append(
  ...
  <div class="site-shell">
    <header class="topbar">
      ...
      <nav class="nav-links">
        ...
        <a href="{_REPO_URL}" target="_blank" rel="noopener">GitHub</a>
      </nav>
    </header>
...
def build_pages_site(root_dir: Path) -> Path:
    ...
    global _SITE_URL, _REPO_URL
    import os
    _SITE_URL = os.environ.get("PUBLIC_SITE_URL", "").rstrip("/")
    _REPO_URL = os.environ.get("REPORT_REPO_URL", "").rstrip("/")
    ...
    json_feed = _build_json_feed(reports, site_url=_SITE_URL)
    rss_feed = _build_rss(reports, site_url=_SITE_URL)
